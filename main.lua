...
task.spawn(function()
	...
	local function handleButtonClick(Button: ImageButton)
		print("3handle button click", Button.ClassName, Button:GetFullName())
	...
	LocalPlayer.PlayerGui.ChildAdded:Connect(function(child)
		if child.Name == "shakeui" and child:IsA("ScreenGui") then
			...
			child.safezone.ChildAdded:Connect(function(grandchild)
				if AutoShakeEnabled and CurrentAction == "Shaking" and grandchild.Name == "button" and grandchild:IsA("ImageButton") then
					handleButtonClick(grandchild)
				end
			end)
			if AutoShakeEnabled and CurrentAction == "Shaking" and child.safezone:FindFirstChild("button") and child.safezone.button:IsA("ImageButton") then
